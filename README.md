...
2. **Di tahap mana terjadi drop-off terbesar** dalam funnel pembelian — baik dari sisi volume maupun drop-off rate?
...
> Terdapat 9.146 duplikat kombinasi user-date, menandakan satu pengguna bisa memiliki lebih dari satu sesi dalam sehari (misalnya dari device atau traffic source berbeda). Data perlu di-*aggregate* ke level user per hari agar funnel analysis tidak bias.

---

## Data Cleaning

Setelah pembersihan data, dataset yang digunakan untuk analisis funnel utama (`df_user`) berisi **102.951 baris** (~11% data merupakan noise / aktivitas tidak valid atau tidak relevan untuk funnel).
...
- Validasi urutan funnel: penghapusan baris tidak logis (checkout > carts, atau purchases > checkouts)
- Menghapus pengguna tidak aktif (tidak ada aktivitas sama sekali)
...
Kolom-kolom ini diterapkan pada dua DataFrame:
- `df_user` — agregasi per user per hari, digunakan untuk analisis funnel overall
- `df_session` — agregasi per user per hari per device & traffic source, digunakan untuk segmentasi
...
<img src="./images/user_dropoffs.png" alt="Drop-off per Tahap" width="500"/>
...
> 🔴 **Biggest drop-off by volume:** Tahap **Page View → Product View** kehilangan **82.425 pengguna** — terbesar secara absolut dan titik prioritas utama untuk optimasi. Lebih dari 80% pengunjung keluar sebelum melihat satu produk pun.
>
> ⚠️ **Highest drop-off rate:** Tahap **Product View → Cart** dengan drop-off rate **83.4%** — artinya dari pengguna yang sudah melihat produk, hanya ~1 dari 6 yang menambahkan ke keranjang.
...
<img src="./images/segment_comparison.png" alt="Segmentasi Device & Traffic Source" width="500"/>

Segmentasi dilakukan menggunakan `df_session`, yaitu agregasi per kombinasi `event_date × user_pseudo_id × device_type × traffic_source`, sehingga satu pengguna yang mengakses dari dua perangkat berbeda dalam satu hari dihitung sebagai dua sesi terpisah.
...
### 1. Prioritas Utama: Perbaiki Page View → Product View
Tahap ini mencatat **drop-off terbesar secara volume** dengan 82.425 pengguna yang keluar sebelum melihat satu produk pun — menjadikannya titik dengan potensi recovery terbesar secara absolut. Temuan ini mengindikasikan bahwa sebagian besar pengunjung belum terdorong untuk mengeksplorasi produk lebih lanjut.
...
### 2. Perhatikan Juga: Product View → Cart
Meski volume pengguna yang hilang lebih kecil (17.104), tahap ini memiliki **drop-off rate tertinggi sebesar 83.4%** — artinya hanya ~1 dari 6 pengguna yang sudah melihat produk yang akhirnya menambahkannya ke keranjang. Ini mengindikasikan adanya friction antara intent dan action pada tahap eksplorasi produk.

**Rekomendasi:**
- Evaluasi kualitas halaman produk: foto, deskripsi, harga, dan ulasan
- Tambahkan CTA yang lebih jelas dan mudah dijangkau (terutama di mobile)
- Pertimbangkan fitur wishlist atau notifikasi stok sebagai jembatan menuju cart

### 3. Optimalkan Pengalaman Mobile & Tablet
...
### 4. Fokus Budget ke Channel Berkualitas Tinggi
...
Lihat analisis lengkap di: [funnel_analysis.ipynb](https://github.com/AdindaAnas/funnel_analysis/blob/main/notebooks/funnel_analysis.ipynb)
